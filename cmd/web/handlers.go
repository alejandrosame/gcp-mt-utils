...
func (app *application) translate(w http.ResponseWriter, r *http.Request) {
    ...
    errorString := "None"
    targetTextString, ok := files.ConvertTextStructToPlainText(targetText)
    if !ok{
        errorString = "translation_errors"
    }
    reply := Reply{Error: errorString, CharactersUsed: fmt.Sprintf("%d", limit.TotalTranslated), Translation: *targetTextString}
    json.NewEncoder(w).Encode(reply)
}
...
func (app *application) translateFile(w http.ResponseWriter, r *http.Request) {
    ...
    sourceText, err := files.ExtractTextToTranslateDocx(tmp_file, true)
    if err != nil {
        app.serverError(w, err)
        return
    }
    ...
    // Unformatted file has the lowest risk of failing
    ...
    files.ArchiveFiles(output_tmp_file_archive, []string{output_tmp_file_no_format, output_tmp_file_interleaved})
...
func (app *application) exportTranslation(w http.ResponseWriter, r *http.Request) {
    ...
    size := files.WriteTranslationInterleavedToDocx(tmpFile,
                                                    files.ConvertPlainTextToTextStruct(sourceText[0]),
                                                    files.ConvertPlainTextToTextStruct(targetText[0]))
